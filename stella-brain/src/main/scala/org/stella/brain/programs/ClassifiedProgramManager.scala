...
object ClassifiedProgramManager {
  ...
  final case class ClassifiedProgramNotification(classified: List[Classified]) extends ClassifiedProgramManagerMessage

  // Client issuing a request to get all classified data that this manager knows on the specified date
  final case class ClassifiedProgramRequest(date: LocalDate, replyTo: ActorRef[ClassifiedPrograms]) extends ClassifiedProgramManagerMessage
  ...
  private def handle(classified: Map[LocalDate, List[Classified]]): Behavior[ClassifiedProgramManagerMessage] =
    Behaviors.setup { context =>
      Behaviors.receiveMessage {
        case ClassifiedProgramNotification(newClassified) =>
          val newClassifiedMap: Map[LocalDate, List[Classified]] = newClassified.groupBy(_._1.time.toLocalDate)
          // WATCH OUT! ++ overrides any existing value so if a given local date exists both in classified and newClassified, the latter will override the former.
          handle(classified++newClassifiedMap)
        case ClassifiedProgramRequest(date, replyTo) =>
